...
This document outlines the plan to enhance the login functionality for the Mythic VTT application, covering both frontend and backend aspects, including OAuth support.
...
-   **Elements**:
    -   **Branding**: Mythic VTT logo/header.
...
-   `GET /auth/{provider}/callback`: Handles provider callback.
    ...
    -   Returns: 302 Redirect to Frontend with JWT (e.g., `https://app.mythicvtt.com/auth/callback?token=xyz`).
