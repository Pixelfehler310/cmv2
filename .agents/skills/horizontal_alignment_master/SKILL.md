...
2. **Master System Map:** Build and maintain the `ISSUES/architecture/01_contracts/overview/HORIZONTAL_MODULE_OVERVIEW.mmd` and `ISSUES/architecture/01_contracts/overview/HORIZONTAL_BACKEND_NAMESPACE_OVERVIEW.mmd` (standalone Mermaid visualizations).
3. **Horizontal Invariants:** Define and enforce rules that must _always_ be true across all related modules (e.g., "A Spell must have a level >= 0").
