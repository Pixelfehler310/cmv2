...
class ContentPackModel(Base, UUIDMixin, TimestampMixin):
    ...
    lifecycle_state: Mapped[str] = mapped_column(String, nullable=False, default="draft", index=True)
    ...
    compatibility_target: Mapped[str | None] = mapped_column(String, nullable=True)
    published_version: Mapped[int | None] = mapped_column(Integer, nullable=True)

    __table_args__ = (
        UniqueConstraint("pack_key", name="uq_dnd5e_content_packs_pack_key"),
        Index("ix_dnd5e_content_packs_lifecycle_homebrew", "lifecycle_state", "is_homebrew"),
    )
...
class CompendiumDefinitionModel(Base, UUIDMixin, TimestampMixin):
    ...
    provenance_author: Mapped[str | None] = mapped_column(String, nullable=True)
    provenance_updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    payload: Mapped[dict] = mapped_column(JSON_PAYLOAD_TYPE, nullable=False, default=dict)

    __table_args__ = (
        CheckConstraint("content_version >= 1", name="ck_dnd5e_compendium_definitions_content_version"),
        UniqueConstraint(
            "pack_id",
            "family",
            "slug",
...
class LinkedEntryModel(Base, UUIDMixin, TimestampMixin):
    ...
    resolve_mode: Mapped[str] = mapped_column(String, nullable=False, default="strict")
    ...
    __table_args__ = (
        ...
        Index("ix_dnd5e_linked_entries_source_relation", "source_definition_id", "relation_kind"),
