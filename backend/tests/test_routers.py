...
async def override_get_db():
    ...
    mock_result_items.scalars.return_value.all.return_value = [
        Item(id="1", name="Test Item", description="Desc", type="Weapon",
             rarity="Common", weight=1.0, price=10, properties={}, effects=[])
    ]
    mock_result_items.scalar_one_or_none.return_value = Item(
        id="1", name="Test Item", description="Desc", type="Weapon", rarity="Common", weight=1.0, price=10, properties={}, effects=[])

    # Mock execute result for Spells
    mock_result_spells = MagicMock()
    mock_result_spells.scalars.return_value.all.return_value = [
        Spell(id="2", name="Test Spell", description="Desc", level=1, school="Evo",
              casting_time="1A", range="30ft", components={}, duration="Inst", effects=[])
    ]
...
async def test_get_monsters(client):
    ...
    mock_result = MagicMock()
    mock_result.scalars.return_value.all.return_value = [
        Monster(id="3", name="Test Monster", description="Desc", size="M", type="Beast", alignment="U", armor_class=10, hit_points=10, hit_dice="1d10", speed={}, strength=10, dexterity=10, constitution=10,
                intelligence=10, wisdom=10, charisma=10, languages="Common", challenge_rating=1.0, xp=100, effects=[], proficiencies=[], special_abilities=[], actions=[], legendary_actions=[], senses={}, inventory=[])
    ]
    mock_session.execute.return_value = mock_result
